...
-- Merkezi konumlandД±rma iГ§in AnchorPoint
gameFrame.AnchorPoint = Vector2.new(0.5, 0)
-- Loading barвЂ™Д±n hemen altД±na, yatayda ortada
...
local function createNebula()
    ...
    spawn(
        function()
            ...
            TweenService:Create(
                nebula,
                TweenInfo.new(moveTime, Enum.EasingStyle.Sine),
                {
                    Position = UDim2.new(
                        nebula.Position.X.Scale + directionX,
                        0,
                        nebula.Position.Y.Scale + directionY,
                        0
                    )
                }
            ):Play()
    ...
    nebula.MouseButton1Click:Connect(
        function()
            score = score + 1
            scoreText.Text = "Score: " .. score
...
-- РЎРёРјСѓР»СЏС†РёСЏ Р·Р°РіСЂСѓР·РєРё (5-7 РјРёРЅСѓС‚)
local percent = 0
local function simulateLoading()
    local totalTime = 0
    local targetTime = math.random(300, 420) -- 5-7 РјРёРЅСѓС‚
